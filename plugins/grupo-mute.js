...
const handler = async ({ sock, m, from, participants, sender, isGroup }) => {
    ...
    if (target === sender) return sock.sendMessage(from, { text: '⚠️ No puedes mutearte a ti mismo' }, { quoted: m })

    const db = getDB()
    if (!db[from]) db[from] = []

    if (db[from].includes(target)) return sock.sendMessage(from, { text: 'ℹ️ Este usuario ya está muteado' }, { quoted: m })
